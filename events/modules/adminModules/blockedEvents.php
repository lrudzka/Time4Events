<?php
    ...
    if (!isset($_GET['dataType']))
    {
        ...
        $eventsQuery = $db->query('SELECT * FROM events_events WHERE blocked =1 AND endDate >= "'.$now.'" ORDER BY startDate, startTime');
    ...
    else
    {
        if ($_GET['dataType']=='archive')
        {
            ...
            //pobieramy eventy z bazy danych
            $eventsQuery = $db->query('SELECT * FROM events_events WHERE blocked=1 AND endDate < "'.$now.'" ORDER BY blockedOn desc, startDate, startTime');
            //odbieramy dane -> do tablicy dwuwymiarowej
            $events = $eventsQuery->fetchAll();
    ...
    if (isset($_POST['province']))
    {
        ...
        if ($allOk)
        {
            ...
            if (!isset($_GET['dataType']))
            {
                ...
                $eventsQuery = $db->query('SELECT * FROM events_events WHERE blocked=1 AND endDate >= "'.$now.'" AND province like "'.$province.'" AND category like "'.$category.'" AND city like "'.$city.'" AND (name like "'.$keyWord.'" OR description like "'.$keyWord.'" OR city like "'.$keyWord.'") AND startDate > "'.$startingFrom.'" AND endDate < "'.$endingTo.'" ORDER BY startDate, startTime');
                //odbieramy dane -> do tablicy dwuwymiarowej
                $events = $eventsQuery->fetchAll();
    ...
    <div class="background">
        <section class="main_width">
            <?php
                ...
                if (!isset($_GET['dataType']))
                {
                    echo '<h4><a href="blockedEvents.php?dataType=archive">Przejdź do wydarzeń archiwalnych</a></h4>';
                }
                else
                {
                    if ($_GET['dataType']=='archive')
                    {
                        echo '<h4><a href="blockedEvents.php">Przejdź do wydarzeń aktualnych</a></h4>';
                    }
                }
                    ...
                    foreach ($events as $event)
                        {
                            ...
                            echo "<div class=".'"col-sm-4 item"'.">
                                ...
                                <div class=".'"buttons"'.">
                                    <span class=".'"btn btn-primary"'."><a href=".'"'."../mainModules/singleEvent.php?id={$event['id']}".'"'." target=".'"_blank"'.">Szczegóły</a></span>
                                    <span><em>Data blokady: </em>{$event['blockedOn']}</span>
                                </div>
                                </div>";    
                        }
                ?>
